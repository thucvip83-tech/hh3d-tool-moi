...
// ============================================================================
// ===== LUYỆN ĐAN ROUTES & LOGIC TỔNG HỢP (FULL CODE BACKEND) =====
// ============================================================================

// 1. Trả về trạng thái hiện tại của Luyện Đan
app.get("/api/ld-state", (req, res) => {
  res.json({ ok: true, running: ldIsRunning, log: readLdLog(), settings: loadLdSettings() })
})

// 2. Xóa lịch sử Log Luyện Đan
app.post("/api/ld-log/clear", (req, res) => { 
  clearLdLogFile(); 
  res.json({ ok: true }) 
})

// 3. Cập nhật các Cài đặt / Setting Luyện Đan
app.post("/api/ld-settings", (req, res) => {
  const cfg = loadConfig()
  const { 
    globalTier, firstTuneDelayMs, tuneDelayMs, profileDelaySec, 
    luyenDanConcurrency, luyenDanBatchStaggerMs, stopMin, profileTiers, 
    ldBuyFeatured, ldBuyBundles, ldBuyRetailEnabled, ldBuyBundleTiers, 
    ldBuyBundleQty, ldBuyRetail, ldBuyHerbs, ldMuaDelayMs, ldUseTiers, 
    ldMeltTiers, ldAutoMeltOnFail, ldAutoMeltTiers 
  } = req.body

  const s = loadLdSettings()
  if (globalTier !== undefined) s.globalTier = globalTier
  if (firstTuneDelayMs !== undefined) s.firstTuneDelayMs = Number(firstTuneDelayMs) || 110000
  if (tuneDelayMs !== undefined) s.tuneDelayMs = Number(tuneDelayMs) || 40000
  if (profileDelaySec !== undefined) s.profileDelaySec = Math.max(1, Number(profileDelaySec) || 3)
  if (luyenDanConcurrency !== undefined) s.luyenDanConcurrency = Math.max(1, Math.min(20, Number(luyenDanConcurrency) || 1))
  if (luyenDanBatchStaggerMs !== undefined) s.luyenDanBatchStaggerMs = Math.max(0, Math.min(10000, Number(luyenDanBatchStaggerMs) || 0))
  if (stopMin !== undefined) s.stopMin = Math.max(0, Number(stopMin) || 0)
  if (ldBuyFeatured !== undefined) s.ldBuyFeatured = ldBuyFeatured
  if (ldBuyBundles !== undefined) s.ldBuyBundles = ldBuyBundles
  if (ldBuyRetailEnabled !== undefined) s.ldBuyRetailEnabled = ldBuyRetailEnabled
  if (ldBuyBundleTiers !== undefined) s.ldBuyBundleTiers = Array.isArray(ldBuyBundleTiers) ? ldBuyBundleTiers : []
  if (ldBuyBundleQty !== undefined) s.ldBuyBundleQty = ldBuyBundleQty
  if (ldBuyRetail !== undefined) s.ldBuyRetail = ldBuyRetail
  
  // ldBuyHerbs — 3 loại thảo dược cao cấp (Linh Phong Thảo / Huyền Vân Thảo / Thiên Đế Thảo)
  if (ldBuyHerbs !== undefined) s.ldBuyHerbs = ldBuyHerbs
  if (ldMuaDelayMs !== undefined) s.ldMuaDelayMs = Math.max(100, Number(ldMuaDelayMs) || 600)
  if (ldUseTiers !== undefined) s.ldUseTiers = ldUseTiers
  if (ldMeltTiers !== undefined) s.ldMeltTiers = ldMeltTiers
  if (ldAutoMeltOnFail !== undefined) s.ldAutoMeltOnFail = ldAutoMeltOnFail
  if (ldAutoMeltTiers !== undefined) s.ldAutoMeltTiers = ldAutoMeltTiers

  // Nếu globalTier được set, áp dụng cho tất cả profile
  if (globalTier !== undefined && globalTier !== "" && Array.isArray(cfg.profiles)) {
    cfg.profiles.forEach(p => {
      if (!p.settings) p.settings = {}
      p.settings.luyenDanTier = globalTier
    })
    saveConfig(cfg)
    broadcast({ type: "config_updated" })
  }

  // profileTiers: { [profileId]: tier }
  if (profileTiers && typeof profileTiers === "object") {
    for (const [pid, tier] of Object.entries(profileTiers)) {
      const p = cfg.profiles.find(x => x.id === pid)
      if (p) { if (!p.settings) p.settings = {}; p.settings.luyenDanTier = tier || "" }
    }
    saveConfig(cfg)
    broadcast({ type: "config_updated" })
  }
  ...
  // Nếu đang chạy, cập nhật luôn ldRuntimeSettings để vòng tiếp theo áp dụng ngay
  if (ldIsRunning) {
    if (profileDelaySec !== undefined) ldRuntimeSettings.profileDelayMs = Math.max(1000, Number(s.profileDelaySec) * 1000)
    if (luyenDanConcurrency !== undefined) ldRuntimeSettings.luyenDanConcurrency = s.luyenDanConcurrency
    if (luyenDanBatchStaggerMs !== undefined) ldRuntimeSettings.luyenDanBatchStaggerMs = s.luyenDanBatchStaggerMs
    if (stopMin !== undefined) ldRuntimeSettings.stopMs = s.stopMin * 60000
  }

  res.json({ ok: true })
})

// 4. Lấy / Cập nhật UI Settings
app.get("/api/ui-settings", (req, res) => {
  res.json({ ok: true, settings: loadUiSettings() })
})

app.post("/api/ui-settings", (req, res) => {
  const s = loadUiSettings()
  const allowed = [
    "manualDelayMs", "manualDelayBetweenTasksMs", "manualStopMin", "manualRepeatMin", "manualRepeatCount",
    "kmDelayInput", "kmGlobalRunMode", "kmRoundDelayMs", "kmRetryFetchDelayMs",
    "kmConcurrentGroupDelay", "kmConcurrentStagger", "kmStopMinInput", "kmCacheToken",
    "kmQuickGroupDelay", "kmQuickStagger", "kmQuickFuncDelay",
    "mcAttackDelay", "mcTotalRounds", "activeTab", "kmUseGroups",
  ]
...
// 5. Lệnh Dừng Luyện Đan
app.post("/api/ld-stop", (req, res) => {
  ldShouldStop = true
  appendLdLog("system", "System", "Da nhan lenh dung, dang ket thuc cac profile trong batch hien tai...")
  appendLdLog("system", "System", "🛑 Đã nhận lệnh dừng, sẽ dừng sau profile hiện tại")
  res.json({ ok: true })
})

// 6. Chức năng 1: Mua linh dược + mở túi (chạy 1 lần, không loop)
app.post("/api/ld-run-mua", async (req, res) => {
  const { profileIds } = req.body
  if (!profileIds?.length) return res.json({ ok: false, error: "Chưa chọn profile" })
  if (ldIsRunning) return res.json({ ok: false, error: "Luyện Đan đang chạy" })
  res.json({ ok: true })

  const Tasks = require("./core/tasks/index.js")
  const cfg = loadConfig()
  const ldSettings = loadLdSettings()

  ldIsRunning = true
  ldShouldStop = false
  broadcast({ type: "ld_start" })
  appendLdLog("system", "System", `── BẮT ĐẦU: Mua Linh Dược + Mở Túi (${profileIds.length} profile) ──`)

  try {
    for (const pid of profileIds) {
      if (ldShouldStop) break
      const _cfg = loadConfig()
      const profile = _cfg.profiles.find(x => x.id === pid)
      if (!profile) { appendLdLog(pid, pid, "Profile không tồn tại"); continue }
      if (!profile.cookies) { appendLdLog(pid, profile.name, "⚠ Chưa có cookie"); continue }
      try {
        appendLdLog(pid, profile.name, "▶ Đang lấy session...")
        const { ctx } = await getContext(profile, _cfg)
        appendLdLog(pid, profile.name, "✅ Session OK")
        const settings = {
          ...ldSettings,
          luyenDanTier: profile.settings?.luyenDanTier || ldSettings.globalTier || "",
        }
        await Tasks.luyenDanMuaVaMo(ctx, settings, (msg) => appendLdLog(pid, profile.name, msg))
      } catch(e) { appendLdLog(pid, profile.name, `❌ Lỗi: ${e.message}`) }
    }
  } catch(e) {
    appendLdLog("system", "System", `❌ Lỗi nghiêm trọng: ${e.message}`)
    const { isNetworkError: _isNetErr } = require("./core/context")
    if (_isNetErr(e)) recordNetworkError(e)
  }

  ldIsRunning = false
  appendLdLog("system", "System", "── KẾT THÚC MUA LINH DƯỢC ──")
...
app.post("/api/ld-run-luyen", async (req, res) => {
  ...
  ldIsRunning = true
  ldShouldStop = false
  const savedSettings = loadLdSettings()
  const delaySec = profileDelaySec !== undefined ? profileDelaySec : savedSettings.profileDelaySec
  const concurrency = luyenDanConcurrency !== undefined ? luyenDanConcurrency : savedSettings.luyenDanConcurrency
  const staggerMs = luyenDanBatchStaggerMs !== undefined ? luyenDanBatchStaggerMs : savedSettings.luyenDanBatchStaggerMs
  const stopAfterMin = stopMin !== undefined ? stopMin : savedSettings.stopMin

  ldRuntimeSettings.profileDelayMs = Math.max(1000, Number(delaySec || 3) * 1000)
  ldRuntimeSettings.luyenDanConcurrency = Math.max(1, Math.min(20, Number(concurrency) || 1))
  ldRuntimeSettings.luyenDanBatchStaggerMs = Math.max(0, Math.min(10000, Number(staggerMs) || 0))
  ldRuntimeSettings.stopMs         = Number(stopAfterMin || 0) * 60000
  ldRuntimeSettings.profileIds     = profileIds

  broadcast({ type: "ld_start" })
  appendLdLog("system", "System", `── BẮT ĐẦU LUYỆN ĐAN: ${profileIds.length} profile, song song ${ldRuntimeSettings.luyenDanConcurrency}, rai ${ldRuntimeSettings.luyenDanBatchStaggerMs}ms, delay ${delaySec || 3}s/batch ──`)
  if (stopAfterMin > 0) appendLdLog("system", "System", `⏱ Tự dừng sau ${stopAfterMin} phút`)

  runLuyenDanLoop(profileIds).finally(() => {
    ldIsRunning = false
    broadcast({ type: "ld_done" })
  })
})

// 8. Hàm Vòng Lặp Điều Phối Luyện Đan (Batch & Concurrency Loop)
async function runLuyenDanLoop(initProfileIds) {
  const Tasks = require("./core/tasks/index.js")
  ...
  while (!ldShouldStop) {
    ...
    let cfg = loadConfig()
    let ldSettings = loadLdSettings()
    const stopMs    = ldRuntimeSettings.stopMs || 0
    const profileIds = ldRuntimeSettings.profileIds || initProfileIds

    if (stopMs > 0 && Date.now() - startedAt >= stopMs) {
      appendLdLog("system", "System", `⏹ Tu dong dung sau ${Math.round(stopMs/60000)} phut`)
      break
    }

    const readBatchCfg = () => {
      const s = loadLdSettings()
      const r = ldRuntimeSettings
      return {
        ldSettings: s,
        cfg: loadConfig(),
        concurrency: Math.max(1, Math.min(20, Number(r.luyenDanConcurrency || s.luyenDanConcurrency) || 1)),
        staggerMs:   Math.max(0, Math.min(10000, Number(r.luyenDanBatchStaggerMs ?? s.luyenDanBatchStaggerMs) || 0)),
        profileDelayMs: r.profileDelayMs || 3000,
      }
    }

    let roundSkippedCount = 0

    const runOneProfile = async (pid, index, batchIndex, batchCfg) => {
      const { cfg: bCfg, ldSettings: bLd, staggerMs } = batchCfg
      const label = `#${String(index + 1).padStart(2, "0")}`

      if (staggerMs > 0 && batchIndex > 0) {
        await waitBatch(staggerMs * batchIndex)
      }
      const profile = bCfg.profiles.find(x => x.id === pid)
      if (!profile) { appendLdLog(pid, pid, `${label} Profile khong ton tai`); return }
      if (!profile.cookies) { appendLdLog(pid, profile.name, `${label} Chua co cookie, bo qua`, 'warn'); return }

      const tier = profile.settings?.luyenDanTier || bLd.globalTier || ""
      if (!tier) {
        appendLdLog(pid, profile.name, `${label} Chua chon pham dan, bo qua`, 'warn')
        return
      }

      try {
        appendLdLog(pid, profile.name, `${label} Bat dau luyen ${tier}`, 'ok')
        const { ctx } = await getContext(profile, bCfg)
        const settings = {
          ...bLd,
          luyenDanTier: tier,
          luyenDanFirstTuneDelay: bLd.firstTuneDelayMs || 110000,
          luyenDanTuneDelay: bLd.tuneDelayMs || 40000,
          _shouldStop: () => ldShouldStop && !ldBatchRunning,
        }

        const result = await Tasks.luyenDan(ctx, settings, (msg, level) => {
          if (msg) appendLdLog(pid, profile.name, `${label} ${msg}`, level)
        })

        const normResult = (result || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[đĐ]/gi,'d')
        const isBagFull     = normResult.includes('tui dan') && normResult.includes('day')
        const isMonthlyDone = normResult.includes('da du dan ca 4 loai')
        const isNoRecipe    = normResult.includes('cong thuc luyen dan khong dung') || normResult.includes('khong co cong thuc')
        const isNoMat       = normResult.includes('khong du linh duoc') || normResult.includes('thieu linh duoc')
        const shouldStop    = isBagFull || isMonthlyDone || isNoRecipe || isNoMat
        const isError       = normResult.includes('loi') || normResult.includes('khong the') || normResult.includes('that bai')
        const resultLevel   = shouldStop ? 'warn' : (isError ? 'err' : 'ok')

        if (shouldStop) roundSkippedCount++
        appendLdLog(pid, profile.name, `${label} ${result || 'Xong'}`, resultLevel)

        try {
          const r = await Tasks.kiemTraDan(ctx, loadLdSettings(), null)
          const entry = { profileId: pid, profileName: profile.name, ...r }
          updateDanCacheEntry(pid, entry)
          broadcast({ type: "dan_cache_updated", profileId: pid, entry })
        } catch (_) {}
      } catch(e) {
        appendLdLog(pid, profile.name, `${label} Loi: ${e.message}`, 'err')
      }
    }

    let bc = readBatchCfg()
    appendLdLog("system", "System", `── Vong ${round} (${profileIds.length} profile, song song ${bc.concurrency}, stagger ${bc.staggerMs}ms) ──`)

    for (let i = 0; i < profileIds.length; i += bc.concurrency) {
      if (ldShouldStop) break
      bc = readBatchCfg()
      const { concurrency, staggerMs, profileDelayMs } = bc
      const batch = profileIds.slice(i, i + concurrency)
      const batchNo = Math.floor(i / concurrency) + 1
      const totalBatch = Math.ceil(profileIds.length / concurrency)

      appendLdLog("system", "System", `Batch ${batchNo}/${totalBatch}: ${batch.length} profile (song song ${concurrency}, stagger ${staggerMs}ms)`)
      ldBatchRunning = true
      await Promise.allSettled(batch.map((pid, j) => runOneProfile(pid, i + j, j, bc)))
      ldBatchRunning = false

      if (!ldShouldStop && i + concurrency < profileIds.length) {
        await waitOrStop(profileDelayMs)
      }
    }

    if (!ldShouldStop) {
      if (roundSkippedCount >= profileIds.length) {
        appendLdLog("system", "System", "⏹ Tất cả profile đều không thể luyện (túi đầy / đủ tháng / không đủ linh dược / sai công thức) — dừng luyện đan.")
        break
      }
      const { profileDelayMs } = readBatchCfg()
      await waitOrStop(profileDelayMs)
    }
  }

  appendLdLog("system", "System", "── KẾT THÚC LUYỆN ĐAN ──")
}
