...
* @returns {Promise} - Một Promise sẽ resolve với token, hoặc null nếu thất bại.
...
* @returns {Promise} Trả về security nonce nếu tìm thấy, ngược lại trả về null.
...
extraControls.push(`${optionsHTML}`);
...
extraControls.push(` title="${quest.toggleTitle}">${isEnabled ? '🔔' : '🔕'}`);
...
const opts = [1,2,3,4,5].map(n => `${n}`).join('');
...
buttonHTML += `${quest.extraButtonText}`;
...
buttonHTML += `${quest.extra2ButtonText}`;
...
Bắt Đầu / Dừng Lại: Chạy tự động tất cả nhiệm vụ.

🤖: Bật/tắt chế độ tự động chạy khi load trang.

🔄 Nút làm mới: Tải lại thông tin xu và tiến độ nhiệm vụ.

⚙️ Cài đặt: Cấu hình chi tiết cho từng nhiệm vụ.

💎 Hấp Thụ: Nhập mã CODE để nhận thưởng.



📋 Danh sách nhiệm vụ:

Nhấn nút hành động (Đánh, Nhận, Khắc,...) để chạy thủ công.

Các nút phụ (📦🙏🌺👑🎁) thực hiện hành động bổ sung.

Bấm vào icon trên mỗi task để bật/tắt tự động chạy mỗi task.

⏳ Thời gian đếm ngược hiển thị khi task đang chờ lượt tiếp.



⚠️ Lưu ý: Đảm bảo đã đăng nhập trước khi sử dụng.
...
Giờ tự khởi động lại hàng ngày:
...
Tự động dừng và chạy lại script vào giờ đã đặt (mặc định 00:30)Bấm lưu lại để áp dụng thay đổi
...
Tối đa hóa sát thương (buff ngũ hành)

Tự động chọn ngũ hành để tăng 15% sát thương
...
Tìm Kẻ Địch (Khoáng Mạch)

ID kẻ địch:

➕ Thêm

ID Tông Môn (cách nhau bằng dấu ;):
...
Không có cài đặt cho nhiệm vụ này
...
Tất cả ✅ Success ⚠️ Warn ❌ Error ℹ️ Info 🔍 Debug
...
if (km_sel) km_sel.innerHTML = '⏳ Đang tải...';
...
km_sel.innerHTML = `-- Chọn mỏ --` + optionsHtml;
...
km_sel.innerHTML = '⚠️ Không tải được danh sách mỏ';
...
ks_enemyList.innerHTML = 'Chưa có ID nào';
...
body: `action=redeem_linh_thach&code=${encodeURIComponent(promoCode)}&nonce=${nonce}<_token=${securityToken}&hold_timestamp=${Math.floor(
...
* @returns {Promise} True nếu câu trả lời được gửi thành công, ngược lại là false.
...
showNotification(`Vấn Đáp: Không tìm thấy đáp án cho câu hỏi: ${question.question}`, 'error');
...
showNotification(`Vấn Đáp: Câu hỏi: ${question.question} không có đáp án đúng trong server.`, 'error');
...
* @returns {Promise} Dữ liệu phiên hoặc null nếu có lỗi.
...
* @returns {Promise} True nếu đặt cược thành công.
...
showNotification(`✅ Cược thành công vào ${stone.name}!Tỷ lệ x${stone.reward_multiplier}`, 'success');
...
* @returns {Promise} True nếu nhận thưởng thành công.
...
`Bạn đã gửi lời chúc phúc cho cặp đôi ${room.user1_name} 💞 ${room.user2_name}`,
...
`Nhận lì xì phòng cưới ${room.wedding_room_id} được ${liXi.data.amount} ${liXi.data.name}!`,
...
* @returns {Array} Mảng chứa các nguyên tố phù hợp.
...
* @returns {Promise} `True` nếu tấn công thành công, ngược lại là `false`.
...
* @returns {Promise} Nguyên tố mới nếu đổi thành công, ngược lại là null.
...
const attacksMatch = html.match(/

Lượt đánh còn lại:\s*(\d+)<\/div>/);
...
const message = `⏳ Cần chờ ${minutes} phút ${seconds} giây để tấn công tiếp.`; ///////////////////
...
const inputRegex = /]+name=["'](?:security|nonce|_wpnonce)["'][^>]*value=["']([^"']+)["']/gi;

while ((mm = inputRegex.exec(html)) !== null) map._global = map._global || mm[1];

const meta = /]+name=["'](?:wpnonce|security|nonce)["'][^>]*content=["']([^"']+)["']/i.exec(html);
...
return `${typePrefix}${mine.name} (${mine.id})`;
...
showNotification(`Nhận thưởng ${reward.data.total_tuvi} tu vi và ${reward.data.total_tinh_thach} tinh thạch tại khoáng mạch ${reward.data.mine_name}`, 'info');
...
showNotification(`Có ${outerUsers.length} người chơi ngoại tông trong mỏ!`, 'warn');
...
showNotification(`Khoáng mạch chưa đủ thời gian.Hiện đạt: ${myInfo.time_spent}Cần: Đạt tối đa`, 'warn');
...
showNotification(`Khoáng mạch chưa đủ thời gian.Hiện đạt: ${myInfo.time_spent}Cần: ${requiredMinutes} phút`, 'warn');
...
showNotification(`[Khoáng mạch] Bonus ${bonus}% chưa đạt ${rewardMode}%Hiện không thể đoạt mỏ.Không thực hiện được hành động nào.`, 'info')
...
* @returns {Promise>} Mảng đối tượng tổng môn
...
${esc(groupDisplay)} - ${esc(roleDisplay)}
...
⚠️ Ngoại Tông Trong Mỏ ${outerUsers.length}
...
${esc(u.name)} ${allyLabel ? `${allyLabel}` : ''} (${id})

${esc(u.tongMonName || 'Vô phái')} - ${esc(u.role || 'Thành viên')}
...
Quân số: ${mine.users.length} | Phe: ${esc(tongList)}
...
🎯 Tìm thấy ${foundUsers.length} mục tiêu
...
Nguồn: ${source} • ${timeSinceStr(timestamp)}
...
// resDiv.innerHTML = `${tuViStr} | ${rightSideHtml}`;
...
` style="display:block; width:100%; margin: 5px 0; background-color: #3085d6;">${opt}`
...
// lấy text từ nếu có, nếu không fallback div (class thay đổi mỗi id) / h4
...
Tu Vi: ${opponentTuViText}

Tỷ Lệ Thắng: ${displayRate}%
...
🩸Kẻ địch: ${totalEnemies}

🤝Liên Minh: ${totalLienMinh}

☯️Đồng Môn: ${totalDongMon}
